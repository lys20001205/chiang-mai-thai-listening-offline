...
(async()=>{
 stores.set('chiangmai-ear-shell-v6',new Map());stores.set('other-app',new Map());
 await lifecycle('install');check(skipped,'Activated after successful caching');
 check(CACHE==='chiangmai-ear-shell-v7','Release cache v7');check(stores.get(CACHE).size===5,'Five assets cached');
 ...
 network='offline';
 check(await(await request(ROOT+'alphabet.html?v=1.2.0')).text()==='ALPHABET','Alphabet fallback is alphabet');
 check(await(await request(ROOT+'practice.html?revision=7')).text()==='PRACTICE','Practice fallback remains distinct');
