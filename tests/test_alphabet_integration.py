 ...
 with sync_playwright() as p:
  ...
  for file in ['index.html','practice.html']:
   ...
   if file=='index.html':
    ...
    check(q.locator('.sign-board .pron').count()==1,'Original sign shows pronunciation')
    ...
    check(q.locator('.board .pron').count()==1,'Practice sign shows pronunciation')
    q.evaluate('setPage("money")');check(q.locator('#money-form').count()==1,'Money form intact')
   check('v1.4.0' in q.locator('body').inner_text(),file+' version')
